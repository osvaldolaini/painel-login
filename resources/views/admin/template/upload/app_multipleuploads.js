var UploadImage = function () {
    ...
    function validarArquivo(file){
        // Tipos permitidos
        var mime_types = ['image/jpg','image/jpeg','image/webp','image/png'];
        // Validar os tipos
        if(mime_types.indexOf(file.type) == -1) {
            Swal.fire({
                icon: 'error',
                title: 'Erro!',
                text: "O arquivo " + file.name + " não permitido",
            })
        ...
        if(file.size > 2*1024*1024) {
            Swal.fire({
                icon: 'error',
                title: 'Erro!',
                text: file.name + " ultrapassou limite de 2MB",
            })
        ...
        $(document).on('change', '.upload-file', function () {
            ...
            for(var i = 0; i < files.length; i++){
                ...
                if(info.error == undefined){
                    ...
                    enviarArquivo(i, barra, input); //Enviar, input da imagem
                }else{
                    text.innerHTML = info.error;
                    barra.classList.add("error");
                }
                //Adicionar barra na div com a cass de mesmo nome da id da imagem
                document.querySelector('.'+input).appendChild(barra);
            };
        });
    }
    /*Realizando o upload da imagem via ajax */
        function enviarArquivo(indice, barra, input){
            var data = new FormData();
            var request = new XMLHttpRequest();
            //Adicionar arquivo
            data.append('file', document.querySelector('#'+input).files[indice]);
            ...
            request.addEventListener('load', function(e) {
                ...
                if(request.response.status == "success"){
                    ...
                    '</div><input type="hidden" name="'+input+'" value="'+request.response.name+'">';
    ...
    let viewImages = function(){
        $(document).on('click', '.showImage', function () {
            ...
            $('body').append('<div class="modal fade" id="imagesModel">'+
                ...
                '</div>'+
              '</div>');
            $('#imagesModel .modal-body').append('<img src="'+path+'/'+image+'" class="mx-auto d-block" style="max-width:100%;">');
